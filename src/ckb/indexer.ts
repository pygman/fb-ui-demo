...
export class CkbIndexer implements Indexer {
  ...
  async waitForSync(blockDifference = 0): Promise<void> {
    ...
    console.debug('rpcTipNumber', rpcTipNumber);
    let index = 0;
    while (true) {
      const indexerTipNumber = parseInt((await this.tip()).block_number, 16);
      console.debug('indexerTipNumber', indexerTipNumber);
      if (indexerTipNumber + blockDifference >= rpcTipNumber) {
        return;
      }
      console.debug(`wait until indexer sync. index: ${index++}`);
      ...
      collect(): CellCollectorResults {
        return {
          async *[Symbol.asyncIterator]() {
            ...
            let cursor = null;
            for (;;) {
              const params = [searchKey, order, `0x${sizeLimit.toString(16)}`, cursor] as any;
              console.debug('get_cells params', params);
...
*/
  ...
  public async getCells(
    ...
    while (true) {
      ...
      const liveCells = res.objects;
      cursor = res.last_cursor;
      console.debug('liveCells', liveCells[liveCells.length - 1]);
  ...
  start(): void {
    console.debug('ckb indexer start');
  }

  startForever(): void {
    console.debug('ckb indexer startForever');
  }

  stop(): void {
    console.debug('ckb indexer stop');
  }
